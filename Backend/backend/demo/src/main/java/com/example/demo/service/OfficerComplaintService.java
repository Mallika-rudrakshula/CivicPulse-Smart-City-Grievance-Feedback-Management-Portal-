...
public class OfficerComplaintService {
   ...
   public Complaint updateStatus(Officer officer, Long complaintId, ComplaintStatus status) {
    ...
    // ✅ SET RESOLUTION DATE AUTOMATICALLY ONLY WHEN FIRST RESOLVED
    if (status == ComplaintStatus.RESOLVED && previousStatus != ComplaintStatus.RESOLVED) {
        ...
        // Check if officer has no more active complaints and set to AVAILABLE
        ...
        if (activeCount == 0) {
            officer.setStatus(com.example.demo.entity.OfficerStatus.AVAILABLE);
